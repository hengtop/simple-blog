 ...
 * @LastEditTime: 2022-06-03 23:34:12
 */
import React, {
  memo,
  useState,
...
export default memo(function Index(props: PropsWithChildren<PropsType>) {
  ...
  const getOffsetTopByHashAndHref = () => {
    ...
    domRenderObjArr.map((item) => {
      ...
      const offsetTopHref = (
        document.querySelector('a[href="' + hash + '"]') as any
      ).offsetTop as number;
  ...
  const setScrollTopForNavigate = (index: number) => {
    if (navigateContainerDom.current) {
      const maxScrollTop =
        navigateContainerDom.current.scrollHeight -
        navigateContainerDom.current.clientHeight;
